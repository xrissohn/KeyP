...
- **AI Pipeline (specialized model per agent role, all via Replit AI Integrations — no user API keys, billed to credits)**:
  ...
  - **Deliverer** = deterministic freshness×confidence sort (no LLM). **Seed guarantee**: when caller asks for `count=1` (initial registration) AND every candidate was filtered by Verifier, the Deliverer rescues the freshest raw candidate at confidence=55 with a "참고용" reason so the user is never shown an empty result on first registration.
